...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    let (model_path, tracks, samples) = arguments("audio2x-regression")?;
...
fn arguments(name: &str) -> Result<(PathBuf, usize, Option<usize>), String> {
    let mut values = std::env::args_os().skip(1);
    let model = values
        .next()
        .map(PathBuf::from)
        .ok_or_else(|| format!("usage: {name} <model.json> [tracks] [samples]"))?;
    let tracks = values
        .next()
