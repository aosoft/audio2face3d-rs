use crate::{Audio2xModel, ModelKind, ModelParameters};
use audio2emotion::{
...
use audio2x_core::{
    Audio2xError, AudioAccumulator, EmotionAccumulator, GeometryAudioParameters,
    GeometryParameters, NetworkDocument, Result,
};
...
impl TensorRtPipeline {
    pub fn load(model: &Audio2xModel, options: PipelineOptions) -> Result<Self> {
...
fn geometry_input_strength(model: &Audio2xModel, index: usize) -> Result<f32> {
...
fn accumulator_error(error: impl std::fmt::Display) -> Audio2xError {
...
mod tests {
    ...
    fn runs_installed_models_through_facade_when_configured() {
        let Some(paths) = std::env::var_os("AUDIO2X_TEST_FACADE_MODELS") else {
            return;
        };
        for root in std::env::split_paths(&paths) {
            let model = Audio2xModel::load(root.join("model.json")).unwrap();
