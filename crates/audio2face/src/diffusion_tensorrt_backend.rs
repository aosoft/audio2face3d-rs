use crate::{DiffusionBackend, DiffusionContract, DiffusionFrameInput, DiffusionInferenceOutput};
...
mod tests {
    ...
    fn runs_installed_diffusion_model_when_configured() {
        let Some(root) = std::env::var_os("AUDIO2X_TEST_DIFFUSION_MODEL_DIR") else {
            return;
        };
