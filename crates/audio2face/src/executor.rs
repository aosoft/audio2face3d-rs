use crate::{RegressionContract, RegressionFrameInput};
use audio2x_core::{Audio2xError, AudioAccumulator, EmotionAccumulator, Result};
...
impl RegressionExecutor {
    ...
    fn track_ready(&self, track: &RegressionTrack<'_>, frame: usize) -> Result<bool> {
        ...
        let audio_end = window
            ...
            .ok_or_else(|| Audio2xError::InvalidSchema("audio window overflow".into()))?;
    ...
    fn drop_consumed(&self, track: &RegressionTrack<'_>, frame: usize) -> Result<()> {
        ...
        track.audio.drop_samples_before(
            usize::try_from(next.start.max(0))
                .map_err(|_| Audio2xError::InvalidSchema("negative drop watermark".into()))?,
        )?;
        track
            .emotions
            .drop_before(current.target)
            .map_err(|error| Audio2xError::InvalidSchema(format!("emotion drop failed: {error}")))
    }
    ...
    pub fn wait(&self) -> Result<()> {
        ...
        while state.running {
            state = self
                .idle
                .wait(state)
                .map_err(|_| Audio2xError::InvalidSchema("executor mutex poisoned".into()))?;
        }
        Ok(())
    }
...
mod tests {
    use super::*;
    use audio2x_core::{RegressionAudioParameters, RegressionParameters};
