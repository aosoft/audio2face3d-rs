use audio2x_core::{
    Audio2xError, AudioAccumulator, Binding, BindingSchema, Dimension, ElementType,
    EmotionAccumulator, IoMode, RegressionAudioParameters, RegressionParameters, Result, Shape,
    WindowProgress, WindowProgressParameters,
};
...
impl RegressionContract {
    pub fn new(
        ...
        let stride_numerator = audio.samplerate.checked_mul(frame_rate_denominator).ok_or(
        ...
        let target_offset =
            i64::try_from(audio.buffer_ofs).map_err(|_| Audio2xError::IntegerOverflow {
    ...
    pub fn prepare_frame(
        ...
        emotion.extend(emotions.read(window.target).map_err(|error| {
            Audio2xError::InvalidSchema(format!("emotion is unavailable: {error}"))
        })?);
