...
pub use audio2face3d_inference::audio::{FPS, FrameBuffer, SAMPLE_RATE};
use tonic::Status;
pub fn validate_header(header: Option<&AudioHeader>) -> Result<(), Status> {
    ...
    let format = audio2face3d_protocol::convert::decode_audio_format(*header)
        .map_err(crate::backend::status)?;
    audio2face3d_inference::config::validate_format(format).map_err(crate::backend::status)
}
