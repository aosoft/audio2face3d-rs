...
impl Shape {
    ...
    pub fn element_count(&self, batch: usize, dynamic: &[usize]) -> Result<usize> {
        ...
        for dimension in &self.0 {
            let value = match *dimension {
                ...
                Dimension::Dynamic { min, max } => {
                    let value = *dynamic.get(dynamic_index).ok_or_else(|| {
                        Audio2xError::InvalidSchema("missing dynamic dimension".into())
                    })?;
                    dynamic_index += 1;
        ...
        if dynamic_index != dynamic.len() {
            return Err(Audio2xError::InvalidSchema(
                "too many dynamic dimensions".into(),
            ));
        }
        Ok(count)
    }
    ...
    pub fn new(bindings: impl Into<Vec<Binding>>) -> Result<Self> {
        ...
        for binding in &bindings {
            ...
            if !names.insert(binding.name.clone()) {
                return Err(Audio2xError::DuplicateBinding(binding.name.clone()));
            }
        }
        Ok(Self(bindings))
    }
