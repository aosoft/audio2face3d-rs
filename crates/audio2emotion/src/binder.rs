use crate::EmotionCallbackMetadata;
use audio2x_core::{Audio2xError, EmotionAccumulator, Result};
...
impl<'a> EmotionBinder<'a> {
    ...
    pub fn accumulate(&self, metadata: EmotionCallbackMetadata, emotions: &[f32]) -> Result<bool> {
        ...
        self.accumulators
            .get(metadata.track)
            .ok_or_else(|| {
                Audio2xError::InvalidSchema("emotion binder track is out of range".into())
            })?
            .accumulate(metadata.timestamp, emotions)
            .map_err(|error| {
                Audio2xError::InvalidSchema(format!("emotion binder accumulate failed: {error}"))
            })?;
        Ok(true)
    }
}
