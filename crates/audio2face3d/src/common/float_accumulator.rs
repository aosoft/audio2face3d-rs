...
impl FloatAccumulator {
    ...
    pub fn accumulate(&self, samples: &[f32]) -> Result<()> {
        if samples.is_empty() {
            return Err(Error::InvalidSchema("empty samples".into()));
        }
        let mut state = self.lock()?;
        if state.closed {
            return Err(Error::InvalidSchema("accumulator is closed".into()));
        }
    ...
    pub fn close(&self) -> Result<()> {
        let mut state = self.lock()?;
        if state.closed {
            return Err(Error::InvalidSchema("accumulator is already closed".into()));
        }
        state.closed = true;
        Ok(())
    }
    ...
    pub fn read(&self, start: i64, length: usize) -> Result<Vec<f32>> {
        if length == 0 {
            return Err(Error::InvalidSchema("empty destination".into()));
        }
        let state = self.lock()?;
        let end = start
            .checked_add(i64::try_from(length).map_err(|_| Error::IntegerOverflow {
                field: "read_length",
                value: length,
                target: "i64",
            })?)
            .ok_or(Error::InvalidSchema("read range overflow".into()))?;
    ...
    pub fn drop_samples_before(&self, start: usize) -> Result<()> {
        let mut state = self.lock()?;
        if start > state.next {
            return Err(Error::InvalidSchema("cannot drop future samples".into()));
        }
    ...
    fn lock(&self) -> Result<std::sync::MutexGuard<'_, State>> {
        self.state
            .lock()
            .map_err(|_| Error::InvalidSchema("accumulator mutex poisoned".into()))
    }
}
