...
impl WindowProgress {
    ...
    pub fn window(&self, index: usize) -> Result<SampleWindow> {
        let scaled =
            index
                .checked_mul(self.parameters.stride_numerator)
                .ok_or(Error::IntegerOverflow {
                    field: "window_stride",
                    value: index,
                    target: "usize",
        ...
        let start = self
            .parameters
            .start_offset
            .checked_add(i64::try_from(scaled).map_err(|_| Error::IntegerOverflow {
                field: "window_start",
                value: scaled,
                target: "i64",
            })?)
            .ok_or_else(|| Error::InvalidSchema("window start overflow".into()))?;
        let target = start
            .checked_add(self.parameters.target_offset)
            .ok_or_else(|| Error::InvalidSchema("window target overflow".into()))?;
        let end = start
            ...
            .ok_or_else(|| Error::InvalidSchema("window end overflow".into()))?;
    ...
    pub fn available_windows(&self, end_timestamp: Timestamp, closed: bool) -> Result<usize> {
        let additional = if closed {
            ...
            i64::try_from(self.parameters.window_size).map_err(|_| Error::IntegerOverflow {
                field: "window_size",
                value: self.parameters.window_size,
                target: "i64",
            })?
        ...
        let offset = self
            ...
            .ok_or_else(|| Error::InvalidSchema("window offset overflow".into()))?;
        ...
        let distance = end_timestamp
            ...
            .ok_or_else(|| Error::InvalidSchema("window availability overflow".into()))?;
        let mut count = div_ceil_signed(distance, stride);
        let position = offset
            .checked_add(
                count
                    .checked_mul(stride)
                    .ok_or_else(|| Error::InvalidSchema("window count overflow".into()))?
                    / denominator,
            )
            .and_then(|value| value.checked_add(limit))
            .ok_or_else(|| Error::InvalidSchema("window limit overflow".into()))?;
