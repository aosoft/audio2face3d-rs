...
use crate::protocol::wire::{a2f, controller};
use crate::types::{
...
pub fn decode_request(value: controller::AudioStreamHeader) -> Result<RequestOptions> {
    ...
    result.blendshapes = value.blendshape_params.map(|p| BlendshapeParameters {
        multipliers: p.bs_weight_multipliers.into_iter().collect(),
        offsets: p.bs_weight_offsets.into_iter().collect(),
        clamp: p.enable_clamping_bs_weight,
    });
    result.emotion = value.emotion_params.map(|p| EmotionParameters {
        transition_time: p.live_transition_time,
        beginning: p.beginning_emotion.into_iter().collect(),
    });
    result.emotion_post_processing = value
        .emotion_post_processing_params
        .map(|p| {
            let value = EmotionPostProcessing {
                contrast: p.emotion_contrast,
                smoothing: p.live_blend_coef,
                use_preferred: p.enable_preferred_emotion,
                preferred_strength: p.preferred_emotion_strength,
                strength: p.emotion_strength,
                max_emotions: p
                    .max_emotions
                    .map(|n| u32::try_from(n).map_err(|_| protocol("negative max emotions")))
                    .transpose()?,
            };
            Ok::<_, crate::types::Error>(value)
        })
        .transpose()?;
