...
use crate::protocol::wire::{animation, controller};
use crate::types::{
...
pub fn encode_stream_info(value: StreamInfo) -> Result<controller::AnimationDataStreamHeader> {
    if value.audio_format.is_none() && value.curves.is_none() {
        return Err(crate::types::Error::invalid(
    ...
    let epoch = value
        ...
        .map(|v| {
            v.duration_since(UNIX_EPOCH)
                .map(|d| d.as_secs_f64())
                .map_err(|_| crate::types::Error::invalid("start time before epoch"))
        })
        .transpose()?
        .unwrap_or(0.0);
...
pub fn encode_animation(value: OutputBatch) -> Result<animation::AnimationData> {
    ...
    for frame in value.curves {
        ...
        if layout.as_ref().is_some_and(|l| **l != *current) {
            return Err(crate::types::Error::invalid("mixed curve layouts"));
        }
        if last.is_some_and(|p| time <= p) {
            return Err(crate::types::Error::invalid("non-increasing curve times"));
        }
        layout = Some(current);
        last = Some(time);
    ...
    let audio = value
        ...
        .map(|block| {
            ...
            Ok::<_, crate::types::Error>(animation::AudioWithTimeCode {
