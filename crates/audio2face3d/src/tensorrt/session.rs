...
mod tests {
    ...
    fn loads_real_engine_metadata_when_configured() {
        let Some(path) = std::env::var_os("AUDIO2FACE3D_TEST_ENGINE") else {
            eprintln!("skipped: AUDIO2FACE3D_TEST_ENGINE is not configured");
            return;
        };
    ...
    fn resolves_dynamic_output_shapes_when_configured() {
        let Some(path) = std::env::var_os("AUDIO2FACE3D_TEST_DYNAMIC_ENGINE") else {
            eprintln!("skipped: AUDIO2FACE3D_TEST_DYNAMIC_ENGINE is not configured");
            return;
        };
        let device = GpuDevice::new(0).unwrap();
        let stream = device.create_stream().unwrap();
    ...
    fn rejects_a_stream_from_another_device_when_available() {
        let Some(engine) = std::env::var_os("AUDIO2FACE3D_TEST_ENGINE") else {
            eprintln!("skipped: AUDIO2FACE3D_TEST_ENGINE is not configured");
            return;
        };
