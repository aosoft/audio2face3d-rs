use crate::animation::{RegressionBackend, RegressionContract, RegressionFrameInput};
...
fn validate_schema(schema: &BindingSchema, contract: &RegressionContract) -> Result<()> {
    let expected = [
        (
            "emotion",
            crate::common::IoMode::Input,
            contract.emotion_size,
        ),
        ("input", crate::common::IoMode::Input, contract.audio_size),
        (
            "result",
            crate::common::IoMode::Output,
    ...
    for (name, mode, width) in expected {
        let binding = schema
            .get(name)
            .ok_or_else(|| Error::InvalidSchema(format!("missing regression binding {name}")))?;
