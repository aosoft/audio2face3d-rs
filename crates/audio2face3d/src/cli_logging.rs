//! Shared executable logging setup. Only applications should install a subscriber.
mod worker;
use crate::logging::{LogLevel, LogRecord, LogValue, Logger};
...
fn open_output(args: &LogArgs, prefix: &str) -> io::Result<Box<dyn Write + Send>> {
...
fn json_line(
    ...
    let level = format!("{level:?}").to_ascii_lowercase();
    ...
    pub fn start(
        ...
        if prefix.is_empty()
            || !prefix
                ...
                .all(|c| c.is_ascii_alphanumeric() || b"_-".contains(&c))
        {
            return Err("log filename prefix must contain only ASCII letters, digits, underscores or hyphens".into());
        }
        let output = open_output(args, prefix)?;
...
mod tests {
    ...
    fn text_envelope_order() {
        ...
        let subscriber = tracing_subscriber::fmt()
            .with_ansi(false)
            .with_target(false)
            .with_timer(Timezone::Utc)
            .with_writer(move || writer.clone())
            .finish();
        tracing::subscriber::with_default(subscriber, || {
            tracing::info!(target: "audio2face3d_log", message = %"preparing inference", fields = ?vec![("source", "test")]);
        });
        let text = String::from_utf8(bytes.lock().unwrap().clone()).unwrap();
        let time = text.split_whitespace().next().unwrap();
        chrono::DateTime::parse_from_rfc3339(time).unwrap();
        assert!(!text.contains("audio2face3d_log"));
        assert!(text.find("INFO").unwrap() < text.find("preparing inference").unwrap());
        assert!(text.find("preparing inference").unwrap() < text.find("fields=").unwrap());
    }
    ...
    fn timezone_and_directory_outputs() {
        ...
        for format in [Format::Json, Format::Text] {
            for timezone in [Timezone::Utc, Timezone::Local] {
                ...
                for _ in 0..2 {
                    let mut output = open_output(&args, env!("CARGO_PKG_NAME")).unwrap();
                ...
                assert!(open_output(&invalid, env!("CARGO_PKG_NAME")).is_err());
            }
        }
    ...
    fn json_preserves_types_and_escaping() {
        ...
        assert!(text.starts_with(
            "{\"timestamp\":\"1970-01-01T00:00:00.123+00:00\",\"level\":\"info\",\"message\":"
        ));
        assert!(text.find(",\"message\":").unwrap() < text.find(",\"fields\":").unwrap());
