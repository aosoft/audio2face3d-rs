...
use crate::{audio::FrameBuffer, resample::Resampler};
#[test]
...
fn stack_normalization_matches_reference_including_wrapped_storage() {
    ...
    for offset in [0, 2, 200] {
        for count in [1, 17, 400] {
            ...
            crate::audio::normalized_samples(&bytes, offset, &mut scratch[..count]);
...
fn identity_weights_move_and_permuted_weights_keep_clamping() {
    ...
    let weights = crate::animation::ordered_weights(weights, &[0, 1, 2], true);
    assert_eq!(weights.as_ptr(), pointer);
    assert_eq!(weights, [0.0, 0.75, 1.0]);
    assert_eq!(
        crate::animation::ordered_weights(vec![-0.5, 0.75, 1.5], &[2, 0, 1], false),
        [1.5, -0.5, 0.75]
    );
    assert_eq!(
        crate::animation::ordered_weights(vec![-0.5, 0.75, 1.5], &[2, 0, 1], true),
        [1.0, 0.0, 0.75]
    );
}
...
fn native_staging_profile() {
    ...
    for _ in 0..iterations {
        ...
        crate::audio::normalized_samples(black_box(&pcm), 0, &mut samples);
    ...
    for _ in 0..iterations {
        ...
        let values = crate::animation::ordered_weights(values, &(0..52).collect::<Vec<_>>(), true);
