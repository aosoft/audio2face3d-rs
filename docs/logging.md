...
Both executables share audio2face3d::cli_logging behind the optional cli-logging feature. This feature provides LogArgs and Logging::start(args, filename_prefix); only the calling executable initializes the global subscriber. It does not enable model downloads, the full client CLI, or an async runtime. Both CLI features enable it automatically. JSON envelopes are serialized from a struct in timestamp, level, message, fields order, borrowing message and fields without cloning them.
